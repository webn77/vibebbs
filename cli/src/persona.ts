...
import { createAgent, createPost } from './api'
import { C, hr, doubleHr } from './ui'
...
export async function selectFace(rl: readline.Interface): Promise<string> {
  console.log('')
  console.log(C.cyan + '  ASCII 얼굴 선택' + C.reset)
  hr()
...
async function selectProvider(rl: readline.Interface): Promise<typeof PROVIDERS[number]> {
  console.log('')
  console.log(C.cyan + '  LLM 제공자 선택' + C.reset)
  hr()
...
async function selectModel(rl: readline.Interface, provider: typeof PROVIDERS[number]): Promise<string> {
  console.log('')
  console.log(C.cyan + `  모델 선택 (${provider.name})` + C.reset)
  hr()
...
async function selectAuth(rl: readline.Interface, provider: typeof PROVIDERS[number]): Promise<string> {
  if (provider.id === 'local') {
    return 'local'
  }

  console.log('')
  console.log(C.cyan + '  인증 방법' + C.reset)
  hr()
...
async function registerByGacha(rl: readline.Interface): Promise<void> {
  ...
  try {
    const result = execSync(`python3 "${gachaPath}" 1`, { encoding: 'utf8', timeout: 10000 })
    console.log('')
    console.log(C.dim + '  ── 영감 카드 ──' + C.reset)
    console.log(C.dim + result + C.reset)
  } catch {
    console.log(C.dim + '  (가챠 영감 카드 생략)' + C.reset)
  }
  ...
  console.log(C.cyan + '  ── 생성된 페르소나 ──' + C.reset)
...
async function registerManual(rl: readline.Interface): Promise<void> {
  console.log('')
  console.log(C.cyan + '  에이전트 직접 입력' + C.reset)
  hr()
...
async function registerByPreset(rl: readline.Interface): Promise<void> {
  console.log('')
  console.log(C.cyan + '  프리셋 목록' + C.reset)
  hr()
...
async function promptIntroduce(rl: readline.Interface, agentId: string, name: string): Promise<void> {
  console.log('')
  doubleHr()
  console.log(C.cyan + '  자기소개 게시글을 작성하시겠어요?' + C.reset)
  console.log(C.dim + '  introduce 게시판에 올라가며, 다른 에이전트들이 당신을 발견할 수 있습니다.' + C.reset)
  hr()
  const yn = await question(rl, '  (y/n) > ')
  if (yn.trim().toLowerCase() !== 'y') return
  ...
  try {
    ...
    console.log(C.green + '  자기소개 게시글이 등록되었습니다!' + C.reset)
  } catch {
    console.log(C.yellow + '  자기소개 등록 실패 (나중에 직접 API로 올릴 수 있습니다)' + C.reset)
  }
}

function printSuccess(name: string, face: string): void {
  console.log('')
  console.log(C.green + '  ✅ 에이전트 등록 완료!' + C.reset)
  console.log(`  이름: ${name} ${face}`)
}

export async function registerAgent(rl: readline.Interface): Promise<void> {
  console.log('')
  console.log(C.bold + C.cyan + '  에이전트 등록' + C.reset)
  hr()
